...
function addToCart(id, name, price, image = '', colors = [], productId = null, combiIdx = null, maxQty = null) {
  ...
  if (existing) {
    if (maxQty !== null && maxQty !== undefined && existing.qty >= maxQty) {
      showToast('Maximale verfügbare Menge erreicht!', 'error');
      return;
    }
    existing.qty++;
  } else {
    if (maxQty !== null && maxQty !== undefined && maxQty <= 0) {
      showToast('Leider ausverkauft!', 'error');
      return;
    }
    cart.push({ id, name, price, image, colors, qty: 1, productId, combiIdx, maxQty });
  }
