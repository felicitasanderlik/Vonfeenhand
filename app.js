...
function addToCart(id, name, price, image = '', colors = []) {
  const existing = cart.find(i => i.id === id);
  if (existing) { existing.qty++; }
  else { cart.push({ id, name, price, image, colors, qty: 1 }); }
  saveCart(); updateCartUI();
  showToast(`"${name}" zum Korb hinzugefügt${colors.length ? " · " + colors.length + " Farben" : ""} 🧶`);
